...
class FlowlineModel(object):
    ...
    def run_until_and_store(self, y1, run_path=None, diag_path=None,
                            store_monthly_step=None, stop_criterion=False):
        ...
        for i, (yr, mo) in enumerate(zip(monthly_time, months)):
            ...
            if stop_criterion:
                if yr > 20:
                    yr_idx = int(yr)
                    vol_subset = diag_ds.volume_m3.values[yr_idx-20:yr_idx]
#                    print('yr:', yr, vol_subset.sum())
                    if vol_subset.sum() == 0:
#                        print('stopping because nothing left')
                        break
                
                if yr > 100:
                    yr_idx = int(yr)
                    vol_4mb = diag_ds.volume_m3.values
                    area_4mb = diag_ds.area_m2.values
                    spec_mb = (vol_4mb[1:] - vol_4mb[:-1]) / area_4mb[:-1] * cfg.PARAMS['ice_density']
                    spec_mb_avg = spec_mb[yr_idx-100:yr_idx].mean()
#                    print(yr, 'spec_mb_avg:', np.round(spec_mb_avg,2))
                    if np.abs(spec_mb_avg) < 10:
#                        print('stopping because reached equilibrium')
                        break                    
