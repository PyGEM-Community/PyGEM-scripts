...
option_massloss_bsl = False                       # Number of glaciers that are not connected to the ocean, but have mass below sea level (e.g., Iceland)
...
#%%
if option_massloss_bsl:
    
    glacier_stats_fp = '/Users/drounce/Documents/HiMAT/spc_backup/nsidc/glacier_stats/mass_bsl_annual/'
    
    glacnos_bsl = []
    for reg in regions:
        for rcp in ['ssp245']:
            
            bsl_fn = 'R' + str(reg).zfill(2) + '_glac_mass_bsl_annual_c2_ba1_50sets_2000_2100-' + rcp + '.nc'
            
            ds = xr.open_dataset(glacier_stats_fp + str(reg).zfill(2) + '/' + bsl_fn)
            
            glacnos_reg = [x.split('-')[1] for x in list(ds.RGIId.values)]
            main_glac_rgi_reg = modelsetup.selectglaciersrgitable(glac_no=glacnos_reg)
            rgiids_reg = list(main_glac_rgi_reg.RGIId.values)
            
            bsl_init = np.mean(ds.glac_mass_bsl_annual.values,axis=0)[:,0]
            bsl_init_all = bsl_init.sum()
            
            bsl_idx = np.where(bsl_init > 0)[0]
            if len(bsl_idx) > 0:
                main_glac_rgi_bsl = main_glac_rgi_reg.loc[bsl_idx,:]
                main_glac_rgi_bsl.reset_index(inplace=True, drop=True)
                
                main_glac_rgi_bsl_nontw = main_glac_rgi_bsl.loc[main_glac_rgi_bsl['TermType']-1 == 0, :]
                
                ds_idx_bsl_nontw = []
                if main_glac_rgi_bsl_nontw.shape[0] > 0:
                    for rgiid in main_glac_rgi_bsl_nontw['RGIId'].values:
                        ds_idx_bsl_nontw.append(rgiids_reg.index(rgiid))
            
                    bsl_init_nontw = bsl_init[ds_idx_bsl_nontw].sum()
                    print(reg, np.round(bsl_init_nontw / bsl_init_all,2), bsl_init_all)
                
                else:
                    print(reg, 0, bsl_init_all)
